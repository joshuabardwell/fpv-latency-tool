...
### Added
- Manual transition editing. Detection and edge measurement are heuristics that no single set of thresholds reads correctly on every clip, so the first pass is now a starting point the user can correct. Select a transition marker (Up/Down while reviewing, or click it on the graph or a results row), then move it a frame at a time with Shift+←/→, drop it onto the playhead with M, switch between its first-light and fully-lit ends with Shift+↑/↓, or delete the whole transition with Delete so a false read stops stealing the match from the real transition beside it. Every key has a button in the new Transition Editing panel under the graph, which shows both measured frames, the automatic value beside any you have changed, and how far through the clip you are. Plain ←/→ still only move the playhead, so you can step either side of a marker to check it against the video without losing hold of it.
- Manual edits outrank the algorithm and survive every parameter change. An edit is bound to the transition rather than to a position in a list, so changing Edge Sensitivity, Min ΔBrightness or Min Spacing re-measures everything *except* what you placed by hand. An edit whose transition a threshold change hides is kept dormant and comes back when the transition does. Hand-placed markers are outlined in white on the graph, deleted transitions stay visible as a dim grey ×, and an edited pair gets a ✎ in the results table. CSV export is unchanged.
- Per-clip settings file, `<video>.latency.json`, written beside the footage after the first analysis and restored when the clip is reopened: FPS, both ROIs, direction, all four detection parameters, the in/out points and every manual edit. Reopening a clip picks a half-finished review back up where it left off. CLI flags are applied on top of the restored settings, so a flag you type overrides the file and a flag you omit comes from it; `--no-sidecar` ignores and writes nothing. Restoring never starts an analysis on its own.
...
- Three latency metrics per pair instead of one: **First pixel** (display first-light minus source first-light) and **Full frame** (display fully-lit minus *source first-light*, not source fully-lit) share the source's own first-light frame as their zero point, so full-frame latency can never read below first-pixel latency; **Average** is the mean of the two. Results tables gained First/Avg/Full (ms) columns and each direction's summary grew from one row to three, one per metric. CSV export carries all three in frames and ms.
...
- Measurement-quality warnings, in two registers that answer different questions. **Per transition** — *is this number trustworthy?* — Low contrast, Ambiguous start, Never settles and Drifting levels. Flagged rows get a ⚠ whose tooltip explains what was seen and why it matters, rather than naming the check. **Per ROI** — *what shape is this signal?* — whether the baseline or contrast varies across the clip. That one reads as information, not a warning, unless some measurement is flagged too: a baseline that moves is normal when the device under test has auto-exposure, and the measurements compensate for it. The tool never asserts *why* a baseline moved, since auto-exposure, ROI framing, changing light and a nudged camera are indistinguishable in the signal. Warnings never suppress or auto-exclude anything, and CSV export carries the raw check names so an export stays greppable.
...
### Fixed
- Full-frame latency could read *below* first-pixel latency for the same pair when a source's own plateau bounced for a few frames after its steepest step (real 240fps footage of a flickering LED measured 37.5ms first-pixel against 29.2ms full-frame). Root cause was in noise estimation: pre- and post-transition residuals were pooled into one MAD, so a quiet baseline diluted a noisy plateau's real scatter down to near-zero sigma, which kept the settling band too tight until noise happened to cross it several frames late. Local sigma now takes the larger of the baseline's and the plateau's own MAD rather than pooling them, and full-frame latency is now anchored at the source's first-light frame (not its own fully-lit frame) so the ordering is guaranteed rather than incidental — see "The three metrics" in DESIGN.md.
...
- Clicking a detection-parameter spinbox (Max Latency, Min ΔBrightness, Min Spacing, FPS, Known Period) then clicking the graph, timeline, or video preview no longer leaves the spinbox eating every navigation key (arrows, Home/End, I/O, etc.) — those widgets are all `NoFocus` and clicking one doesn't reclaim focus from whatever last held it. A mouse click anywhere outside the focused spinbox now releases its focus.

### Changed
- Manual Exclude ticks are now cleared when the set of pairs actually changes, rather than on every recompute. Previously any redetect cleared them, which would have wiped a review pass's exclusions on every marker nudge; nudging a marker cannot re-pair anything (pairing keys on the steepest-change point, which nudging never touches), so those ticks now survive it. A deletion, a threshold change that moves transitions, and clicking Analyze all still clear them. Side effect worth knowing: changing Edge Sensitivity no longer clears exclusions, which it used to do despite pairing being identical.
- Escape clears the transition selection when no analysis is running. It still cancels a running analysis, which takes precedence.
